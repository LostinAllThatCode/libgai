...
external update_and_render_(UpdateAndRender)
{
	...
	dt_history_max = window->info.width;
	if (time > 5.f)	a = -1;
	if (time <= 0.f) a = 1;
	...
	gairb_setup perspective = { PerspectiveProjection((float) window->info.width / (float) window->info.height, (float) fov, 0.1f, 1000.f) * CameraOrbit(cam_along_z, GAIRB_DEG2RAD(-15.f), GAIRB_DEG2RAD(0)) * modelview };
	...
	gairb_setup test = { OrthographicProjection((r32)window->info.width, (r32)window->info.height, -2, 2) * Translate(ZRotation(GAIRB_DEG2RAD(rotY)) , V3(window->info.width/2.f, window->info.height/2.f, 0.f)) };
	gairb_PushSetup(&g1, &test);

	gairb_PushRect(&g1, &platform->assets[2], V4(1.f, 1.f, 1.f, 1.0f), V3(0.f, 0.f, 0.f), window->info.width/2.f, window->info.height/2.f, gairb_AlignToCenter);
	#endif

	gairb_setup ortho = { OrthographicProjection((r32)window->info.width, (r32)window->info.height, -2, 2) };
